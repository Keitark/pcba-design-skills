...
![実際の読みにくい nescart 回路図と読みやすい回路図を切り替える Schematic Humanizer アニメーションバナー](assets/banner.gif)
...
必要な場合は、バス速度、長さ差、電源容量、基準面、USB/RF、アンテナキープアウト、メーカー資料も `[SPEC]`（仕様）、`[TARGET]`（設計目標）、`[TBD-MEASURE]`（要実測）に区分して追加します。
...
6. 必要に応じて、根拠を区分したレイアウト注記と資料リンクを追加します。
7. 全ページをページ全体と高密度部分の両方で目視確認し、注記追加後も重なりがないか再確認します。
8. 電気的な懸念は別項目で報告します。読みやすい回路図が正しい回路とは限りません。
9. PDF／画像のみの結論には「目視ベース・未検証」と明記します。
